...
{
	if (!pCuerpo || !pSprite) {
		throw std::invalid_argument("Avatar constructor received null pointer");
	}
...
void Avatar::SetVisualScale(float scaleX, float scaleY) {
	if (m_pSprite) {
		// Mantenemos la escala física original pero modificamos la visual
		b2AABB aabb;
		// ... (cálculo del tamaño original como en el constructor)
		float physToVisualX = scaleX * 2.0f * aabb.GetExtents().x / m_pSprite->getTexture()->getSize().x;
		float physToVisualY = scaleY * 2.0f * aabb.GetExtents().y / m_pSprite->getTexture()->getSize().y;
		m_pSprite->setScale(physToVisualX, physToVisualY);
	}
}

void Avatar::SetVisualRotation(float degrees) {
	if (m_pSprite) {
		// Convertimos a radianes para Box2D y mantenemos la rotación física
		float radians = degrees * b2_pi / 180.0f;
		m_pCuerpo->SetTransform(m_pCuerpo->GetPosition(), radians);
		// SFML usa grados para la rotación visual
		m_pSprite->setRotation(degrees);
	}
}
...
{
	if (!m_pSprite) {
		throw std::runtime_error("Attempted to access null sprite");
	}
	return *m_pSprite;
}
bool Avatar::IsValid() const
{
	// Verifica que ambos punteros internos sean válidos
	return m_pCuerpo != nullptr && m_pSprite != nullptr;
}
