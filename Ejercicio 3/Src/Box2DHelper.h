...
public:
	...
	static b2Body* CreateRectangularStaticBody(b2World* phyWorld, float sizeX, float sizeY)
	{
		...
		b2FixtureDef box = CreateRectangularFixtureDef(sizeX, sizeY, 0.0f, 0.0f, 1.0f);
		body->CreateFixture(&box);
		return body;
	}
	...
	//-------------------------------------------------------------
	static b2MouseJoint* CreateMouseJoint(b2World* phyWorld, b2Body* groundBody, b2Body* targetBody, const b2Vec2& targetPos, float maxForce, float frequencyHz
		= 5.0f, float dampingRatio = 0.7f)
	{
		...
		return (b2MouseJoint*)phyWorld->CreateJoint(&jointDef);
		/*b2MouseJointDef jointDef;
